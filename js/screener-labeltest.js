...
dd.make.scale.color = function() {
    ...
    .range(['#901a18', '#ac3f3b','#d87c74','#77b259', '#2c8333', '#1b451f'])
	;
}
...
dd.draw.dropdown = function(o) {
	...
	$.each(dd.param.criteria, function(name, type) {
		if (o.paramInclude) {
			if (!type[o.paramInclude]) { var exclude = true ; }
		}
...
dd.draw.sectorRing = function() {
	var data = [100,100,100,100,100,100,100,100,100]

	var w = 960,
	    h = 500,
	    r = Math.min(dd.param.w, dd.param.h) / 2,
	...
	var svg = dd.container.append('svg:g')
		.attr("transform", "translate(" + dd.param.w / 2 + "," + dd.param.h / 2 + ")")
	;

    dd.arcs = svg.selectAll("g.slice")
		.data(donut(data))
		.enter().append("svg:g")
		.attr("class", "slice");

    dd.arcs.append("svg:path")
	...
	dd.arcs.text = dd.arcs.append("svg:text")
		.attr('transform', function(d) { 
			var a = angle(d);
			if (a>90 && a<270) { return 'rotate(180 '+arc.centroid(d)+')' }
			else { return null }
		})
		.attr('x', Math.PI*(r-10)*2 / data.length / 2)
		.attr("dy", "15px")
//		.attr("transform", function(d) { console.log(arc.centroid(d)); return "translate(" + arc.centroid(d) + ")"; })
	...
	dd.arcs.text.append('svg:textPath')
...
dd.draw.circles = function() {
	...
	// 		if(!d.exclude) {
	// 			d3.select(this).style('stroke','#fff').style('stroke-width',1/dd.param.scale);
...
//			.style('font-size', function(d,i) { dd.scale.text(d[dd.param.size]) })
	...
	dd.force.on('tick', function(e) {
		dd.circleGroup.selectAll('circle')
				.attr('cx', function(d) { return d.x })
				.attr('cy', function(d) { return d.y })
		dd.symbolGroup.selectAll('text')		
				.attr('x', function(d) { return d.x })
				.attr('y', function(d) { return d.y + dd.scale.text(d[dd.param.size])*.4 })
	});
};
